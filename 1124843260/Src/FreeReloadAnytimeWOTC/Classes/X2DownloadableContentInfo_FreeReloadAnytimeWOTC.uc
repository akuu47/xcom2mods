...
class X2DownloadableContentInfo_FreeReloadAnytimeWOTC extends X2DownloadableContentInfo;
/// <summary>
...
static event OnPostTemplatesCreated()
{
	...
	local X2WeaponUpgradeTemplate			WeapUpgrade;

	ItemTemplateManager = class'X2ItemTemplateManager'.static.GetItemTemplateManager();

	WeaponUpgrades = ItemTemplateManager.GetAllUpgradeTemplates();

	foreach WeaponUpgrades(WeapUpgrade)
	{
		if (WeapUpgrade.FreeReloadCostFn != none)
		{
			WeapUpgrade.FreeReloadCostFn = none; // No longer use existing mechanic
			WeapUpgrade.FriendlyRenameFn = none; // Don't rename normal reload
			WeapUpgrade.BonusAbilities.AddItem('FreeReload');
			`log("Free reload upgrade patched for" @ WeapUpgrade.DataName,, 'FreeReloadAnytime');
		}
	}
}
