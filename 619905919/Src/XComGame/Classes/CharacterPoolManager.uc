		...
		if( Indices.Length != 0 )
		{
			...
			foreach `XCOMHISTORY.IterateByClassType(class'XComGameState_Unit', Unit )
			{
				...
				for( i=0; i<Indices.Length; i++ )
				{
					...
					else if (CharacterPool[Indices[i]].GetLastName() ~= "UNIFORM")
					{
						RemoveValue = Indices[i];
						`log("REMOVING UNIFORM CHARACTER");
					}
					...
					{
						`log("CHARACTER POOL POSSIBILITY" @ CharacterPool[Indices[i]].GetFullName());
					}
				}
			}
