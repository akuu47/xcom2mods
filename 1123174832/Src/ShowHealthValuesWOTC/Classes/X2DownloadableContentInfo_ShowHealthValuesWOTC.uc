...
class X2DownloadableContentInfo_ShowHealthValuesWOTC extends X2DownloadableContentInfo;
...
exec function SHVToggleNumbers()
{
	class'NewHealthBar'.default.SHOW_VALUE = !class'NewHealthBar'.default.SHOW_VALUE;

	`PRES.m_kUnitFlagManager.ForceRefreshAllUnitFlags();

	`log("Unitflag class:" @ string(`PRES.m_kUnitFlagManager.m_arrFlags[0].class),, 'NewHealthBar');
}

exec function TestSpawnFlag()
{
	local UIUnitFlag ff;

	ff = `PRES.Spawn(class'UIUnitFlag', `PRES);
	`log("Unitflag class on SPAWN:" @ string(ff.class),, 'NewHealthBar');
