class X2Effect_SpawnLostZombie extends X2Effect_SpawnUnit;
...
function OnSpawnComplete(const out EffectAppliedData ApplyEffectParameters, StateObjectReference NewUnitRef, XComGameState NewGameState, XComGameState_Effect NewEffectState)
{
	...
	{
		`RedScreen("X2Effect_SpawnLostZombie: This cannot run on a read only object: " $DeadUnitGameState.ObjectID);
	}
...
{	
	...
	local XComGameState_Effect SpawnLostZombieEffect;
	...
	SpawnLostZombieEffect = EffectTargetUnit.GetUnitAffectedByEffectState(EffectName);
	`assert(SpawnLostZombieEffect != none);
	...
	CopyDeadUnitAction.ReanimatorAbilityState = XComGameState_Ability(History.GetGameStateForObjectID(SpawnLostZombieEffect.ApplyEffectParameters.AbilityInputContext.AbilityRef.ObjectID));
...
defaultproperties
{
	UnitToSpawnName="LostZombie"
	AltUnitToSpawnName="LostZombieHuman"
	TurnedZombieName="TurnedIntoLostZombie"
	EffectName="SpawnLostZombieEffect"
	AnimationName="HL_GetUpA"
	bClearTileBlockedByTargetUnitFlag=true
	bCopyTargetAppearance=true
	StartAnimationMinDelaySec=5.5f
	StartAnimationMaxDelaySec=6.5f
}