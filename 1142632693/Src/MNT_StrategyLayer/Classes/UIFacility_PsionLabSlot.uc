...
class UIFacility_PsionLabSlot extends UIFacility_StaffSlot
...
simulated function OnPersonnelSelected(StaffUnitInfo UnitInfo)
{
	...
	local UIChoosePsionAbility PsiScreen;
	...
	{
		HQPres = `HQPRES;
		if (HQPres.ScreenStack.IsNotInStack(class'UIChoosePsionAbility'))
		{
			PsiScreen = Spawn(class'UIChoosePsionAbility', self);
			PsiScreen.m_UnitRef = UnitInfo.UnitRef;
			PsiScreen.m_StaffSlotRef = StaffSlotRef;
			HQPres.ScreenStack.Push(PsiScreen);
		}
		
		//`HQPRES.UIChoosePsionAbility(UnitInfo.UnitRef, StaffSlotRef);
	}
	else
	{
