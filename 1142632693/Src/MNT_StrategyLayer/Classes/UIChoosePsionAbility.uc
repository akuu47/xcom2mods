...
class UIChoosePsionAbility extends UISimpleCommodityScreen config(Mint_StrategyOverhaul);

var array<SoldierAbilityInfo> m_arrAbilities;

var config int NumPsiPerksPerLevel;
...
simulated function array<Commodity> ConvertAbilitiesToCommodities()
{
	...
	for (iAbility = 0; iAbility < m_arrAbilities.Length; iAbility++)
	{
		...
		{
			bPausedProject = class'MNT_Utility'.static.HasPausedPsiAbilityTrainingProject(m_UnitRef);
...
simulated function int GetAbilityOrderDays(int iAbility)
{
	...
	PsiProject = class'MNT_Utility'.static.GetPausedPsiAbilityTrainingProject(m_UnitRef);

	if (PsiProject != None)
	{
		`LOG("MNT_Utility: UIChoosePsionAbility: Found a paused training project. Reprojecting hours remaining.");
	...
	else
	{
		...
		RankDifference = Max(m_arrAbilities[iAbility].iRank - ExtraState.GetPsionRank(), 0);
...
{
	if (class'MNT_Utility'.static.HasPausedPsiAbilityTrainingProject(m_UnitRef))
	{
...
simulated function array<SoldierAbilityInfo> GetAbilities()
{
	...
	local XComGameState_Unit Unit;
	local XComGameState_HeadquartersProject_TrainPsion AbilityProject;
	local array<name> AddedAbilityNames;
	local int iName;
	local name AbilityName;
	local bool bAddAbility;
	local XComGameState_Unit_Additional ExtraState;
	local array<PsionAbilityConfig> PsiAbilities;
	local PsionAbilityConfig ProgressAbility;
	...
	AbilityProject = class'MNT_Utility'.static.GetPsiTrainingProject(m_UnitRef);
	if (AbilityProject != none && AbilityProject.bForcePaused)
	{
		`LOG("MNT_Utility: UIChoosePsionAbility: Found a paused training project. Not allowing any additional choices.");
		// Only add the paused ability to the list as a choice to resume
		AbilityTemplate = class'X2AbilityTemplateManager'.static.GetAbilityTemplateManager().FindAbilityTemplate(AbilityProject.AbilityName);
		...
		AddedAbilityNames.AddItem(AbilityProject.AbilityName);
	}
	else
	{
		// Otherwise generate a list of ability choices
		PsiAbilities = class'MNT_Utility'.static.GetAbilityTree();

		if (!class'MNT_Utility'.static.CanLearnNewPsiSkill(Unit.GetRank(), ExtraState.GetPsionRank()))
			`Redscreen("LP REDSCREEN: should not be able to pick skills at this time.");


		foreach PsiAbilities(ProgressAbility)
		{
			bAddAbility = false;
			
			// Only get appropriate rank abilities
			if(ProgressAbility.Rank <= ExtraState.GetPsionRank())
			{
				AbilityName = ProgressAbility.AbilityName;

				if (AbilityName != 'TheGift' && AbilityName != '' && !ExtraState.HasPsionAbility(AbilityName))
				{	
					...
					if (AbilityTemplate != none)
					{
						...
						if (AbilityTemplate.PrerequisiteAbilities.Length > 0)
						{
							for (iName = 0; iName < AbilityTemplate.PrerequisiteAbilities.Length; iName++)
							{
								...
								if (!ExtraState.HasPsionAbility(AbilityName)) // if the soldier does not have the prereq ability, replace it
								{
									AbilityTemplate = class'X2AbilityTemplateManager'.static.GetAbilityTemplateManager().FindAbilityTemplate(AbilityName);
				...
				//if (class'MNT_Utility'.static.IsGauntletAbility(AbilityName) && Unit.GetSoldierClassTemplateName() == 'Templar');
					//bAddAbility = false;

				if (bAddAbility)
				{
					SoldierAbility.AbilityTemplate = AbilityTemplate;
					SoldierAbility.iRank = ProgressAbility.Rank;
					SoldierAbility.iBranch = ProgressAbility.Option;
	...
	// Randomly shrink to the correct size
	while(SoldierAbilities.Length > default.NumPsiPerksPerLevel){
		SoldierAbilities.Remove(`SYNC_RAND(SoldierAbilities.Length), 1);
	}
...
function bool OnAbilitySelected(int iOption)
{
	...
	local XComGameState_HeadquartersProject_TrainPsion TrainPsiOpProject;
	...
	if (StaffSlotState != none)
	{
		...
		TrainPsiOpProject = class'MNT_Utility'.static.GetPausedPsiAbilityTrainingProject(m_UnitRef);
		if (TrainPsiOpProject != None)
		{
			`LOG("MNT_Utility: UIChoosePsionAbility: Found a paused training project. Resuming.");
			TrainPsiOpProject = XComGameState_HeadquartersProject_TrainPsion(NewGameState.ModifyStateObject(TrainPsiOpProject.Class, TrainPsiOpProject.ObjectID));
			TrainPsiOpProject.bForcePaused = false;
		}
		
		else{
			// Otherwise start a new psi ability training project
			`LOG("MNT_Utility: UIChoosePsionAbility: Did not find a paused training project. Creating new project.");

			TrainPsiOpProject = XComGameState_HeadquartersProject_TrainPsion(NewGameState.CreateNewStateObject(class'XComGameState_HeadquartersProject_TrainPsion'));
			...
			TrainPsiOpProject.AbilityName = m_arrAbilities[iOption].AbilityTemplate.DataName;
