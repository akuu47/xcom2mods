class X2Effect_Vigilance extends X2Effect_Persistent;
...
function bool PostAbilityCostPaid(XComGameState_Effect EffectState, XComGameStateContext_Ability AbilityContext, XComGameState_Ability kAbility, XComGameState_Unit SourceUnit, XComGameState_Item AffectWeapon, XComGameState NewGameState, const array<name> PreCostActionPoints, const array<name> PreCostReservePoints)
{
	...
	{
		if (kAbility.GetMyTemplateName() == 'StandardShot' || kAbility.GetMyTemplateName() == 'SniperStandardFire')
		{
			if (SourceUnit.NumActionPoints() == 0)
			{
				...
				if (PrimaryWeapon.Ammo > 1)
				{
					...
					`XEVENTMGR.TriggerEvent('VigilanceTriggered', AbilityState, SourceUnit, NewGameState);
				}
			}
		}
	}
